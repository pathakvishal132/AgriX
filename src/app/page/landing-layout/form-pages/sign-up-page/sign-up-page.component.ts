...
export class SignUpPageComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    const errorMessages = {
      username: `:@@signUp.error.username:Please provide a valid username`,
      email: `:@@signUp.error.email:Please provide a valid email address`,
      password: `:@@signUp.error.password:Please provide a valid password (min length 6)`,
      confirmPassword: `:@@signUp.error.confirmPassword:Please provide the same value for confirm password`
    };

    if (this.form.invalid) {
      // for (const controlName of Object.keys(this.form.controls)) {
      //   const control = this.form.controls[controlName] as AbstractControl;
      //   if (control.invalid) {
      //     this.toastService.error(errorMessages[controlName], '');
      //   }
      // }
